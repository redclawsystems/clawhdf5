...
use clawhdf5::{AttrValue, FileBuilder, LazyFile};
use clawhdf5_io::prefetch::PrefetchReader;
use clawhdf5_io::{MemoryReader, MmapReadWrite, MmapReader};
...
fn lazy_file_dataset_not_found() {
    ...
    assert!(matches!(err, clawhdf5::Error::Format(_)));
}
...
fn large_file_mmap_correctness() {
    ...
    let file2 = clawhdf5::File::open(&path).unwrap();
