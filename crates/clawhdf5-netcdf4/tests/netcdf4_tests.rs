...
//!
//! We create NetCDF-4 style HDF5 files using clawhdf5's FileBuilder with the
...
use clawhdf5_netcdf4::{Dimension, FillValue, NcType, NetCDF4File};
...
fn make_simple_netcdf4() -> Vec<u8> {
    ...
    b.set_attr(
        "history",
        AttrValue::String("created by clawhdf5-netcdf4 test".into()),
    );
...
fn test_nc_properties() {
    ...
    assert!(props.unwrap().contains("clawhdf5"));
}
...
fn test_global_attrs() {
    ...
    assert!(matches!(
        attrs.get("history"),
        Some(AttrValue::String(s)) if s.contains("clawhdf5-netcdf4")
    ));
}
